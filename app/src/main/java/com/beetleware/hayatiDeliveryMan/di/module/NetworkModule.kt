...
import com.beetleware.hayatiDeliveryMan.common.utils.Localization
import com.beetleware.hayatiDeliveryMan.common.utils.NetworkUtils
...
import com.beetleware.hayatiDeliveryMan.BuildConfig
...
class NetworkModule {
    ...
    fun provideHeadersInterceptor(pref: SharedPreferences): Interceptor {
        ...
        return Interceptor { chain ->
            ...
            val x ="Bearer "+pref.get(Constants.USER_ACCESS_TOKEN_KEY, Constants.server_Auth)
            Log.d("TAG", "provideHeadersInterceptor: $x")
            request = request.newBuilder()
                .header("Accept", "application/json")
                .header("lang", Localization.getLanguage())
                .header("Authorization",pref.get(Constants.USER_ACCESS_TOKEN_KEY, Constants.server_Auth))
                .build()
            chain.proceed(request)
        }
    }
