...
class SubscriptionOrdersAdapter (
    private val incomingOrders: ArrayList<HomeRespone.SubscriptionResponseModel.Data>, val homeView: HomeView, val context: Context
) : RecyclerView.Adapter<SubscriptionOrdersAdapter.IncomingOrdersViewHolder>() {
    ...
    override fun onBindViewHolder(holder: IncomingOrdersViewHolder, position: Int) {
        holder.binding.containerForOrder.setOnClickListener {
            homeView.openOrderDetails(incomingOrders[position].subscription_id,"subscription")
        }
        ...
        holder.binding.tvOrderNumber.text = context.getString(R.string.order_hash)+incomingOrders[position].subscription_id
    }
